...
GameSDLRenderer::GameSDLRenderer(std::shared_ptr<ThreadSafeGameState> state_ptr,
                                 std::shared_ptr<SDL_Renderer> renderer_ptr, std::unique_ptr<GameRunner> game_runner)
...
void GameSDLRenderer::handleEvents_(uint64_t size_x, uint64_t size_y, bool& running, bool& paused) {
    SDL_Event event;
...
void GameSDLRenderer::drawField_() {
    ...
    for (uint32_t i = 0; i < field.size(); ++i) {
        for (uint32_t j = 0; j < field[i].size(); ++j) {
            if (field[i][j]) {
                const SDL_Rect rect{static_cast<int>(kCellPixels * j), static_cast<int>(i * kCellPixels), kCellPixels,
                                    kCellPixels};
                SDL_FRect frect{};

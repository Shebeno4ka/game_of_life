...
public:
    ...
    public:
        ...
        Handle(SandNetworkDriver* driver) : driver_(driver) {}
        const FromClientData& fromClientData() const {return driver_->fromClientData_;}
        const FromServerData& fromServerData() const {return driver_->fromServerData_;}
        void sendToServer(std::vector<CellChange> cells) {this->driver_->sendToServer(std::move(cells));}
    };
private:
